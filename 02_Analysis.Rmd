...
cor_map <- wi_map %>%
  as.data.frame() %>%
  select(first2full_na, abol2, abol, commst, noslave) %>%
  subset(!is.na(first2full_na))

cor(cor_map, use = "pairwise.complete.obs")


m22 <- lm(first2full_na ~ gdplog  + period1 + period1*polity2i_first  +  commst + abol*noslave + noslave , data = wi_df)
```
